...
class Robotmove(threading.Thread): 
   ...
   def __init__(self,shared, shared2, shared3, shared4, shared5): 
          ...
          self.tick          = False
          self.Validation    = False   # GNSS signal validation 
   ...
   def GNSSvalidation(self):                        
       ...
       if(self.tick==True):
           ...
           if len(self.shared1.GPSTtime)>0: 
              # RecTimeS=self.get_sec(self.shared1.GPSTtime[:-4])
               # GNSS reciver time is updated
                   ...
                   if((self.shared1.Q==1)or(self.shared1.Q==2))and(self.shared1.ns>=8)and(self.shared1.ratio>=1.0)and( self.shared1.ValidConn==True):  ## add: sdn, sde - stabilitys for 3s (<0.08) 
                        self.Validation=True
                        #print("GNSS valid", self.Validation )
                   else:
                        print("GNSS data not valid (Q, ns, ratio, TCP_conn):",self.shared1.Q, self.shared1.ns,self.shared1.ratio, self.shared1.ValidConn  )
                        self.Validation=False    
               # else:
               #     self.Validation=False                              
               #     print("FAULT: GNSS Time not changing")                      # GNSS receiver or RTKlib TCP stream fault
               #     print("rectime:",RecTimeS) 
           ...
           self.tick=False 
           self.shared3.GNSS_valid=self.Validation
   ...
   def SpeedMeasure(self):
       ...
       if (self.shared2.PacketCnt>self.oldpacketcnt):                     # @100 ms RX update
           ...
           Speed_tmp=round(((((diffL+diffR)/2)*impmm_scale)/1000)*10, 2)   #  Linear speed (m/s) 
   ...
   def AutomaticDriveMode(self):
      ...
      if ( self.shared3.start==True)and(self.shared3.stop==False):
          ...
          if ( self.state.target_idx> self.shared3.Max_targetIndex ):  # all  waypints targets acquired, STOP
              ...
              if ( self.shared3.pause==False)and(self.shared3.Safe_zone==True)and(self.shared3.GNSS_valid==True):  
                   self.Motors(self.shared3.SpeedV, self.shared3.SpeedW)
              else:
                   self.Motors(0,0)
                   print("Auto STOP (Pause, Safe_zone, GNNS_validation):", self.shared3.pause, self.shared3.Safe_zone, self.shared3.GNSS_valid )
...
#----------------------------------------------        
   def run (self):  
       ...
       while(1):
          ...
          if (self.shared2.PacketCnt>self.oldpacketcnt2):                    # only after new data from MCU  ( 100 ms) 
                self.LogData()
